    ...
    def assess_prototype_impact(
        ...
        try:
            ...
            if torch.std(orig_flat) > 1e-8 and torch.std(mod_flat) > 1e-8:
                output_correlation = torch.corrcoef(torch.stack([orig_flat, mod_flat]))[0, 1].item()
            else:
                output_correlation = 1.0 if torch.allclose(orig_flat, mod_flat) else 0.0
            
            prediction_change_rate = (original_predictions != modified_predictions).float().mean().item()
            confidence_change = (modified_confidences - original_confidences).mean().item()
            ...
            if pooled_std > 1e-8:
                effect_size = torch.mean(modified_outputs - original_outputs).item() / pooled_std.item()
            else:
                effect_size = 0.0
...
class CounterfactualAnalyzer:
                ...
                with torch.no_grad():
                    ...
                    if prediction == target_class:
                        ...
                        result = CounterfactualResult(
                            ...
                            intervention_description=f"Modified prototype {proto_idx} in layer {layer_name}",
                            success=True,
                            input_perturbation_norm=0.0,  # No input perturbation
                            output_change_norm=torch.norm(output - original_output).item(),
                            confidence_change=F.softmax(output, dim=1).max().item() - F.softmax(original_output, dim=1).max().item()
                        )
                ...
                with torch.no_grad():
                    ...
                    if prediction == target_class:
                        ...
                        result = CounterfactualResult(
                            ...
                            modified_output=output.clone(),
                            modified_prediction=prediction,
                            intervention_description=f"Modified feature {feat_idx} in layer {layer_name}",
                            success=True,
    ...
    def analyze_decision_boundary(
        ...
        if hasattr(layer, 'prototypes'):
            for proto_idx in range(min(layer.prototypes.shape[0], 3)):  # Limit for efficiency
                ...
                for _ in range(num_perturbations):
                    ...
                    boundary_changes += (original_predictions != new_predictions).sum().item()
                ...
                stability = 1.0 - (boundary_changes / (num_perturbations * len(input_samples)))
