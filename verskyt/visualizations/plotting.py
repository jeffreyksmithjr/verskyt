...
def plot_prototype_space(
    ...
    for i, label in enumerate(prototype_labels):
        ax.text(transformed_prototypes[i, 0] + 0.02, transformed_prototypes[i, 1], 
                label, fontsize=12)
    ...
    if features is not None:
        ...
        for i, label in enumerate(feature_labels or []):
            ax.arrow(
                0, 0, transformed_features[i, 0], transformed_features[i, 1],
                head_width=0.05, head_length=0.1, fc='gray', ec='gray', alpha=0.6
            )
            ax.text(transformed_features[i, 0] * 1.1, transformed_features[i, 1] * 1.1, 
                    label, fontsize=10, color='gray', ha='center')
...
def visualize_prototypes_as_data(
    ...
    fig, axes = plt.subplots(num_prototypes, top_k, figsize=(top_k * 2, num_prototypes * 2))
    ...
    for i in range(num_prototypes):
        axes[i, 0].set_ylabel(prototype_labels[i], rotation=0, labelpad=40, 
