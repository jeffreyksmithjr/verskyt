...
class TestMissingIntersectionReductions:
    ...
    def test_gmean_reduction_hand_calculated(self):
        ...
        # x·f = [2, 4], p·f = [4, 2]
        # Intersection memberships: [max(2,0), max(4,0)] and [max(4,0), max(2,0)] = [2, 4] and [4, 2]
...
class TestAdvancedGradientFlow:
    ...
    def test_gradient_flow_with_different_reductions(self):
        ...
        for method in reduction_methods:
            ...
            assert prototypes.grad is not None, f"No gradient for prototypes with {method}"
            assert features.grad is not None, f"No gradient for features with {method}"
            
            assert torch.isfinite(x.grad).all(), f"Non-finite gradient for x with {method}"
            assert torch.isfinite(prototypes.grad).all(), f"Non-finite gradient for prototypes with {method}"
            assert torch.isfinite(features.grad).all(), f"Non-finite gradient for features with {method}"