...
class TestFastXORBenchmark:
    ...
    def test_fast_benchmark_execution(self):
        ...
        method_rates = [v for k, v in analysis.items() if k.startswith('convergence_rate_') and '_' in k[17:]]
    ...
    def test_full_paper_replication(self):
        ...
        for key, expected in paper_targets.items():
            if key in analysis:
                ...
                assert abs(actual - expected) < 0.05, f"{key}: expected {expected}, got {actual}"
