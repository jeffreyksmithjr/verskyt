...
from verskyt.core.similarity import IntersectionReduction, DifferenceReduction
...
class TestTverskyProjectionLayerExtended:
    ...
    def test_non_learnable_alpha_beta_parameters(self, sample_data):
        ...
        assert not layer.alpha.requires_grad, "Alpha should not require gradients when learnable_alpha=False"
        assert not layer.beta.requires_grad, "Beta should not require gradients when learnable_beta=False"
        
        # Verify parameter values
        assert torch.allclose(layer.alpha, torch.tensor(0.7)), "Alpha value should be preserved"
        assert torch.allclose(layer.beta, torch.tensor(0.3)), "Beta value should be preserved"
        
        # Test forward pass works correctly
        output = layer(inputs)
        assert output.shape == (batch_size, num_prototypes), f"Expected shape {(batch_size, num_prototypes)}, got {output.shape}"
        ...
        assert torch.allclose(layer.alpha, torch.tensor(0.7)), "Alpha should not change during backward pass"
        assert torch.allclose(layer.beta, torch.tensor(0.3)), "Beta should not change during backward pass"
    ...
    def test_shared_feature_bank(self, sample_data):
        ...
        assert layer.feature_bank is shared_features, "Layer should use the shared feature bank"
        ...
        assert not torch.allclose(shared_features.grad, torch.zeros_like(shared_features.grad)), "Feature gradients should be non-zero"
    ...
    def test_uniform_initialization(self, sample_data):
        ...
        assert prototypes.std() > 0.01, "Prototypes should have reasonable variance with uniform initialization"
    ...
    def test_normal_initialization(self, sample_data):
        ...
        assert prototypes.std() > 0.01, "Prototypes should have reasonable variance with normal initialization"
    ...
    def test_xavier_normal_initialization(self, sample_data):
        ...
        assert abs(actual_std - expected_std) < 0.5, f"Xavier normal std should be around {expected_std}, got {actual_std}"
    ...
    def test_bias_functionality(self, sample_data):
        ...
        assert layer.bias.shape == (num_prototypes,), f"Bias should have shape {(num_prototypes,)}"
    ...
    def test_parameter_reset_functionality(self, sample_data):
        ...
        assert not torch.allclose(layer.prototypes.data, original_prototypes), "Prototypes should have changed"
        assert not torch.allclose(layer.alpha.data, original_alpha), "Alpha should have changed"
        assert not torch.allclose(layer.beta.data, original_beta), "Beta should have changed"
        
        # Reset parameters
        layer.reset_parameters()
        
        # Parameters should be different from modified values (reset to new random values)
        assert not torch.allclose(layer.prototypes.data, original_prototypes + 1.0), "Prototypes should have been reset"
    ...
    def test_get_prototype_functionality(self, sample_data):
        ...
        for i in range(num_prototypes):
            ...
            assert prototype.shape == (in_features,), f"Prototype {i} should have shape {(in_features,)}"
            assert torch.allclose(prototype, layer.prototypes[i]), f"get_prototype({i}) should return prototypes[{i}]"
    ...
    def test_set_prototype_functionality(self, sample_data):
        ...
        for i in range(num_prototypes):
            ...
            assert torch.allclose(layer.prototypes[i], new_prototype), f"set_prototype({i}) should update prototypes[{i}]"
        
        # Test error handling for invalid indices
        with pytest.raises(IndexError):
            layer.set_prototype(num_prototypes, new_prototype)
        
        # Test with different shape (should work since set_prototype doesn't validate shape)
        ...
        assert torch.allclose(layer.prototypes[0], different_shape_prototype), "set_prototype should update prototype"
    ...
    def test_extra_repr_functionality(self, sample_data):
        ...
        assert "num_prototypes" in repr_str, "Representation should include num_prototypes"
    ...
    def test_comprehensive_gradient_flow_different_configurations(self, sample_data):
        ...
        for config in configurations:
            ...
            # Store original parameters
            original_prototypes = layer.prototypes.data.clone()
            original_alpha = layer.alpha.data.clone() if layer.alpha.requires_grad else None
            original_beta = layer.beta.data.clone() if layer.beta.requires_grad else None
            
            # Backward pass
            loss.backward()
            
            # Check gradients exist for learnable parameters
            assert layer.prototypes.grad is not None, f"Prototypes should have gradients for config {config}"
            
            if layer.alpha.requires_grad:
                assert layer.alpha.grad is not None, f"Alpha should have gradients for config {config}"
            else:
                assert layer.alpha.grad is None, f"Alpha should not have gradients for config {config}"
            
            if layer.beta.requires_grad:
                assert layer.beta.grad is not None, f"Beta should have gradients for config {config}"
            else:
                assert layer.beta.grad is None, f"Beta should not have gradients for config {config}"
            
            if layer.bias is not None:
                assert layer.bias.grad is not None, f"Bias should have gradients when present for config {config}"
    ...
    def test_different_reduction_methods_compatibility(self, sample_data):
        ...
        intersection_methods = [IntersectionReduction.PRODUCT, IntersectionReduction.MIN, 
                              IntersectionReduction.MEAN]
        difference_methods = [DifferenceReduction.IGNOREMATCH, DifferenceReduction.SUBSTRACTMATCH]
        ...
        for int_method in intersection_methods:
            for diff_method in difference_methods:
                ...
                output = layer(inputs)
                assert output.shape == (batch_size, num_prototypes), f"Output shape incorrect for {int_method}, {diff_method}"
                assert torch.all(torch.isfinite(output)), f"Output should be finite for {int_method}, {diff_method}"
                
                # Test gradient flow
                loss = output.sum()
                loss.backward()
                assert layer.prototypes.grad is not None, f"Gradients should exist for {int_method}, {diff_method}"
    ...
    def test_numerical_stability_edge_cases(self, sample_data):
        ...
        assert torch.all(torch.isfinite(output)), "Output should be finite for zero inputs"
        
        # Test with large inputs
        large_inputs = torch.ones(batch_size, in_features) * 1000
        output = layer(large_inputs)
        assert torch.all(torch.isfinite(output)), "Output should be finite for large inputs"
        
        # Test with very small inputs
        small_inputs = torch.ones(batch_size, in_features) * 1e-8
        output = layer(small_inputs)
        assert torch.all(torch.isfinite(output)), "Output should be finite for very small inputs"
        
        # Test with mixed positive/negative inputs
        mixed_inputs = torch.randn(batch_size, in_features) * 100
        output = layer(mixed_inputs)
        assert torch.all(torch.isfinite(output)), "Output should be finite for mixed inputs"
    ...
    def test_batch_size_variations(self):
        ...
        for batch_size in [1, 2, 8, 16, 32]:
            ...
            assert output.shape == (batch_size, num_prototypes), f"Incorrect shape for batch_size={batch_size}"
            assert torch.all(torch.isfinite(output)), f"Output should be finite for batch_size={batch_size}"
            
            # Test gradient flow
            loss = output.sum()
            loss.backward()
            assert layer.prototypes.grad is not None, f"Gradients should exist for batch_size={batch_size}"
    ...
    def test_device_compatibility(self, sample_data):
        ...
        for device in devices:
            ...
            assert output.device.type == device.split(':')[0], f"Output should be on {device}"
            assert output.shape == (batch_size, num_prototypes)
            
            # Test backward pass
            loss = output.sum()
            loss.backward()
            assert layer.prototypes.grad.device.type == device.split(':')[0], f"Gradients should be on {device}"