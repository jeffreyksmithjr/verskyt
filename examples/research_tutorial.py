...
from verskyt.core import tversky_similarity, IntersectionReduction, DifferenceReduction
...
def compare_similarity_methods():
    ...
    print(f"Input shape: {x.shape}, Prototypes: {prototypes.shape}, Features: {features.shape}")
    print()
    ...
    for int_method in intersection_methods[:3]:  # Limit for brevity
        for diff_method in difference_methods:
            ...
            print(f"  {method_name:20} | Range: [{similarity.min():.3f}, {similarity.max():.3f}] | Mean: {similarity.mean():.3f}")
...
def analyze_learned_representations():
    ...
    for epoch in range(500):
        ...
        if (epoch + 1) % 100 == 0:
            print(f"  Epoch {epoch+1:3d}: Loss = {loss.item():.4f}, α = {layer.alpha.item():.3f}, β = {layer.beta.item():.3f}")
    ...
    print(f"  Prototype values: {prototypes.flatten()[:4].numpy()}")  # Show first few values
    ...
    with torch.no_grad():
        ...
        for i in range(4):
            print(f"  {X[i].numpy()} → {y[i]:.0f}     | {final_predictions[i]:.0f}        | {final_output[i]:.3f}")
...
def explore_custom_similarities():
    ...
    for config in configs:
        ...
        print(f"  {config['name']:20} (α={config['alpha']:.1f}, β={config['beta']:.1f}): "
