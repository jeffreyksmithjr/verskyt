...
def demo_model_inspection(manager):
    ...
    for proto in prototypes:
        print(f"  {proto.layer_name}.prototype[{proto.prototype_index}]: "
    ...
    for feat in features[:4]:  # Show first 4 only
        print(f"  {feat.layer_name}.feature[{feat.feature_index}]: "
...
def demo_impact_assessment(manager, test_data):
    ...
    modifications = {
        "small_noise": original_proto.vector + torch.randn_like(original_proto.vector) * 0.1,
        "medium_noise": original_proto.vector + torch.randn_like(original_proto.vector) * 0.5,
    ...
    for scale, impact in sensitivity.items():
        print(f"  Scale {scale:3.1f}: prediction change rate = {impact.prediction_change_rate:.1%}")
...
def demo_feature_grounding(manager, train_data):
    ...
    for grounding in groundings:
        print(f"  {grounding.layer_name}.{grounding.parameter_type}[{grounding.parameter_index}] "
              f"-> '{grounding.concept_name}' (confidence: {grounding.confidence:.2f})")
...
def demo_advanced_grounding(manager, train_data):
    ...
    for grounding in auto_groundings:
        print(f"  Feature {grounding.parameter_index} -> '{grounding.concept_name}' "
