...
def print_paper_comparison(analysis: Dict[str, float]):
    ...
    for display_name, (key, expected) in paper_targets.items():
        if key in analysis:
            ...
            print(f"{display_name:<25} {expected:<8.2%} {actual:<8.2%} {diff:<8.2%} {status}")
        else:
            print(f"{display_name:<25} {expected:<8.2%} {'N/A':<8} {'N/A':<8} {'⚠️ MISS'}")
...
def print_method_rankings(analysis: Dict[str, float]):
    ...
    for key, rate in analysis.items():
        if key.startswith('convergence_rate_') and '_' in key[17:]:  # Skip single method rates
...
def main():
    ...
    print("=" * 50)
    print(f"Total experiments: {FULL_PAPER_CONFIG.total_runs:,}")
    print(f"Estimated runtime: ~2.2 hours")
    ...
    try:
        ...
        print(f"Results validate paper findings: {'✅' if all(abs(analysis.get(f'convergence_rate_{m}_{d}', 0) - r) < 0.05 for (m, d), r in [('product', 'substractmatch', 0.53), ('gmean', 'ignorematch', 0.00)]) else '❌'}")
