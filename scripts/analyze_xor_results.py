...
def analyze_by_method_combination(results: List[Dict]) -> Dict[str, Dict]:
    ...
    for result in results:
        ...
        method_stats[combo]["convergence_rates"].append(
    ...
    for combo, stats in method_stats.items():
        summary[combo] = {
            ...
            "avg_final_loss": (
                statistics.mean(stats["final_losses"])
...
def analyze_by_feature_count(results: List[Dict]) -> Dict[int, Dict]:
    ...
    for result in results:
        ...
        feature_stats[n_features]["convergence_rates"].append(
            1.0 if result["converged"] else 0.0
        )
...
def analyze_by_initialization(results: List[Dict]) -> Dict[str, Dict]:
    ...
    for result in results:
        ...
        init_stats[combo]["convergence_rates"].append(
            1.0 if result["converged"] else 0.0
        )
...
def print_method_analysis(method_summary: Dict[str, Dict]):
    ...
    sorted_methods = sorted(
        method_summary.items(), key=lambda x: x[1]["convergence_rate"], reverse=True
    )

    print(
        f"{'Method Combination':<25} {'Conv Rate':<10} "
        f"{'Avg Acc':<10} {'Avg Loss':<10} {'N':<6}"
    )
    print("-" * 60)

    for combo, stats in sorted_methods:
        print(
            f"{combo:<25} {stats['convergence_rate']:<10.2%} "
...
def print_feature_analysis(feature_summary: Dict[int, Dict]):
    ...
    for n_features, stats in sorted_features:
        print(
            f"{n_features:<10} {stats['convergence_rate']:<12.2%} "
...
def print_paper_comparison(method_summary: Dict[str, Dict]):
    ...
    for combo, paper_rate in paper_targets.items():
        if combo in method_summary:
            ...
            print(
                f"{combo:<25} {paper_rate:<8.2%} "
                f"{actual_rate:<8.2%} {diff:<8.2%} {status}"
            )
        else:
...
def main():
    ...
    parser.add_argument(
        "results_file", type=Path, help="JSON file with detailed results"
    )
    parser.add_argument(
        "--paper-comparison", action="store_true", help="Include paper comparison"
    )
